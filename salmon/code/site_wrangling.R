...
# 3. download Sacramento River flowlines using a bounding box from your stations
...
# flowlines <- st_read(nhd_data, layer = "NHDFlowline", quiet = TRUE) %>%
...
# 'pathlength' in NHD indicates kilometers to the basin outlet
stations_with_rkm <- stations %>%
  mutate(
    ...
    river_km = flowlines$pathlength[nearest_indexes] # flowlines$Pathlength[nearest_indexes]
  ) %>%
