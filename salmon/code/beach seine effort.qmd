---
title: "Beach Seine Spatial & Temporal Effort"
author: "Pete Nelson"
format: pdf
editor: visual
---
...
I generated heatmaps to show the spatial and temporal effort put towards beach seines in the DJFMP. Here are the details regarding how I first checked for data quality issues, then made the heatmaps and some of the decisions I made regarding how sites chosen or removed from consideration.
...
Download DJFMP data; beach seine data are in 'dt3'. Remove the superfluous objects from downloading the data in your workspace.
...
rm(list = c("infile1", "infile2", "infile3", "infile4", "infile5",
            "inUrl1", "inUrl2", "inUrl3", "inUrl4", "inUrl5",
            "tmp1SampleDate", "tmp2SampleDate", "tmp3SampleDate",
            "tmpDateFormat"))
...
Data quality: potential issues with seine volume, turbidity, turbidity_fnu, specific_conductance? All values appear to be within the normal range.
...
# temporal & spatial effort

Regularize the time series, and identify the months for which each site was sampled and *not* sampled. Added descriptions for regions.

```{r}
# effort
# create monthly coverage indicator and retain region
# irregular time series (ie no rows for samples when sites weren't sampled)
coverage_irreg <- raw %>%
  distinct(
...
write_rds(coverage_irreg, here("salmon/data/processed/coverage_irreg.rds"))
...
coverage <-
  ...
  rows_patch(sta_reg, by = "station") %>% 
  # add a month label (as factor)
  mutate(
    month_label = factor(month, levels = 1:12, labels = month.abb)
  ) %>% 
  # add a region label (eg "N Delta")
...
write_rds(coverage, here("salmon/data/processed/coverage.rds"))
...
coverage_irreg <- read_rds(here("salmon/data/processed/coverage_irreg.rds"))
coverage <- read_rds(here("salmon/data/processed/coverage.rds"))

coverage_north <-
  coverage_irreg %>% 
  filter(region %in% c(7,1,2,3),
         year >= 1977) %>% 
  mutate(month_label = 
...
ggplot(coverage_north, 
       aes(month_label, year, fill = factor(sampled))) +
  geom_tile(color = NA) +
  facet_wrap(~station) +
  ...
  labs(
    x = "Month",
    y = "Year",
    title = "Monthly Sampling Coverage: Lower Sac, N Delta & Central Delta"
  ) +
  theme_bw() +
  theme(
...
ggplot(coverage_south, 
  ...
  labs(
    x = "Month",
    y = "Year",
    title = "Monthly Sampling Coverage: S Delta, SJR, SF & SP bays"
  ) +
  theme_bw() +
  theme(
    panel.grid = element_blank(),
...
I basically eye-balled these to select the following sites as judged minimally adequate or better for their temporal coverage.
...
Plot the selected sites to show coverage.

```{r}

good <- read_rds(here("salmon/data/processed/selected_site_codes.rds"))
reg_labs <- read_rds(here("salmon/data/processed/reg_labs.rds"))

# filter coverage data to retain only 'good' sites (station)
selected <-
  # may be better to plot from the irregular time series?
  coverage_irreg %>%
  filter(station %in% good) %>% 
  # add a month label for nicer plot
  mutate(region = factor(region),
         month_label = 
           factor(month, levels = 1:12, labels = month.abb),
         .keep = "unused") %>% 
  left_join(reg_labs, by = "region") %>% 
  # set region (factor) order
  mutate(region_label = factor(region_label,
                               levels = c("Lower Sac", "N Delta", "Cen Delta",
                                          "S Delta", "SJR", "SF & SP Bays"))) %>% 
  # order the stations within each region
  arrange(region_label, station) %>% 
  mutate(station = factor(station, levels = unique(station)))

selected <- selected %>%
  mutate(
    fill_region = ifelse(sampled == 1, as.character(region_label), NA),
    fill_region = factor(fill_region, levels = levels(region_label))
  )

write_rds(selected, here("salmon/data/processed/selected.rds"))

selected <- read_rds(here("salmon/data/processed/selected.rds"))

heatmap_selected <-
  ggplot(selected, 
       aes(month_label, year, fill = fill_region)) +
  geom_tile(color = "grey85") +
  facet_wrap(~station) +
  scale_fill_discrete(na.translate = FALSE, name = "Region") +
  labs(
    x = "Month",
    y = "Year",
    title = "Monthly Sampling Coverage, SELECTED Sites"
  ) +
  theme_minimal() +
  theme(
    panel.grid = element_blank(),
...
ggsave("heatmap_selected.png", 
       path = here("salmon/plots"),
       plot = heatmap_selected, width = 8, height = 6, dpi = 300)

###########
premier <- read_rds(here("salmon/data/processed/premier.rds"))

# filter coverage data to retain only 'premier' sites (station)
premier_selected <-
  # may be better to plot from the irregular time series?
  coverage_irreg %>%
  filter(station %in% premier) %>% 
  # add a month label for nicer plot
  mutate(region = factor(region),
         month_label = 
           factor(month, levels = 1:12, labels = month.abb),
...
premier_selected <- premier_selected %>%
  mutate(
    fill_region = ifelse(sampled == 1, as.character(region_label), NA),
    fill_region = factor(fill_region, levels = levels(region_label))
  )

write_rds(premier_selected, here("salmon/data/processed/premier_selected.rds"))

premier_selected <- read_rds(here("salmon/data/processed/premier_selected.rds"))

heatmap_premier <-
  ggplot(premier_selected, 
       aes(month_label, year, fill = fill_region)) +
  geom_tile(color = "grey85") +
  facet_wrap(~station) +
  scale_fill_discrete(na.translate = FALSE, name = "Region") +
...
Previously, I used the regions used in the DJFMP data, but, for the first flush work, our group has adopted regional boundaries that may differ from these. Here, I'm attempting to sort my beach sites into these regional polygons.

```{r}
# import Dave's regions shapefile
sf_regions <- read_sf(here("data/processed/spatial/delta_regions.shp")) %>% clean_names()

# retrieve DJFMP station metadata
sample_sites <- read_rds(here("salmon/data/raw/dt5.rds"))
...
stations_sf <- st_transform(stations_sf, st_crs(sf_regions))

# spatially join stations to regions
stations_with_regions <- st_join(stations_sf, sf_regions) %>% clean_names()
...
stations_final <- 
  ...
  mutate(region = 
           case_when(
      is.na(region) & lat > 38.5      ~ "SAC",
      is.na(region) & lon > -121.6    ~ "SJR",
      is.na(region) & lon < -122.0    ~ "Bay",
      TRUE                            ~ region))

# check
stations_final %>% 
  filter(region %in% c("SAC", "SJR", "Bay")) %>% 
  select(station, region)

# map w sites outside ff regions gray
ggplot() +
  geom_sf(data = sf_regions, fill = "grey95", color = "grey60") +
  geom_sf(
    data = stations_with_regions,
    aes(color = region),
    size = 2
  ) +
  labs(
...
# map regions include sites outside ff region
ggplot() +
  geom_sf(data = sf_regions, fill = "grey95", color = "grey60") +
  geom_sf(
    data = stations_final,
...
Do the same for the "premier" sites–those with consistent beach seines January-March throughout the time series.
...
# convert stations to an sf object
premier_stations_sf <- 
  st_as_sf(premier_stations,
           coords = c("lon", "lat"),
           crs = 4326) # WGS84 lat/long

# transform station points (NAD83 / UTM zone 10N)
premier_stations_sf <- st_transform(premier_stations_sf, st_crs(sf_regions))

# spatially join stations to regions
premier_stations_with_regions <- st_join(premier_stations_sf, sf_regions) %>% clean_names()

premier_stations_ll <- 
  premier_stations_with_regions %>% 
  st_transform(4326) %>% 
  mutate(
    lon = st_coordinates(.)[,1],
    lat = st_coordinates(.)[,2])

premier_stations_final <- 
  premier_stations_ll %>% 
  mutate(region = 
           case_when(
      is.na(region) & lat > 38.5      ~ "SAC",
...
ggplot() +
  geom_sf(data = sf_regions, fill = "grey95", color = "grey60") +
  geom_sf(
    data = premier_stations_with_regions,
    aes(color = region),
    size = 2
  ) +
  labs(
    title = "Premier Stations Colored by Region",
    color = "Region"
  ) +
  theme_bw()
...
bay_water <- map_dfr(bay_counties, ~ area_water("CA", county = .x, class = "sf")) %>%
...
# bbox <- st_bbox(
  c(xmin = -122.5, ymin = 37.5, xmax = -121.3, ymax = 39.2),
  crs = st_crs(4326)
) %>% st_as_sfc()
...
# get NHD flowlines within the area
flowlines <- get_nhdplus(AOI = bbox, realization = "flowline")

# filter to meaningful streams (avoids tiny ditches cluttering the map)
# flowlines_filtered2 <- flowlines %>%
  filter(streamorde >= 2)  # stream order 2+ = main channels

# try higher stream order
...
ggplot() +
  ...
  # use the bbox object to set map limits
  coord_sf(
    xlim = c(st_bbox(bbox)$xmin, st_bbox(bbox)$xmax),
    ylim = c(st_bbox(bbox)$ymin, st_bbox(bbox)$ymax),
    expand = FALSE
  ) + 
  
  geom_sf(data = sf_regions, fill = "grey95", color = "grey60") +
  
  # Bay/estuary water bodies
  ...
  labs(
    title = "Selected Beach Seine Stations",
    color = "Region"
  ) +
  theme_bw()
...
ggplot() +
  ...
  geom_sf(data = bay_water_filtered, fill = "lightsteelblue2", color = "steelblue3", linewidth = 0.3) +
  geom_sf(data = flowlines_filtered3, color = "steelblue4", linewidth = 0.4, alpha = 0.7) +
  geom_sf(data = premier_stations_final, aes(color = region), size = 2) +
