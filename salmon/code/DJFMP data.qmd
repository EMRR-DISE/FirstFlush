...
Download DJFMP data from EDI using the R script from EDI. I've included version edi.244.14.r here, but check the EDI site for the most current version.
...
Remove unnecessary objects from your workspace, and save the data to a local directory: `salmon/data/raw`.
...
path <- here("salmon/data/raw")
dir_create(path) 

# save objects in a loop
data_list <- mget(paste0("dt", 1:5))
iwalk(data_list, ~ saveRDS(.x, file = file.path(path, paste0(.y, ".rds"))))
